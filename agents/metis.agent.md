...
- **Enforce the Manifest:** If `.agents/atlas.json` exists, it is authoritative and any plan that contradicts it MUST fail. If the manifest is missing, use scoped `AGENTS.md` files as the fallback source of truth and fail plans that contradict them.
...
1. **Rule Extraction:** Use #tool:read to ingest `.agents/atlas.json`. If it is missing, use #tool:search to locate scoped `AGENTS.md` files and read them as the fallback baseline.
...
1. **Manifest Compliance:** Does the plan strictly use the testing framework, linter, and architecture mandated by `.agents/atlas.json`? If the manifest is missing, does it follow the scoped `AGENTS.md` fallback baseline?
